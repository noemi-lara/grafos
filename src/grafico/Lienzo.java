...
public class Lienzo extends JPanel 
            ...
            for (Nodo nodo : vectorNodos) {
                if (new Rectangle(nodo.getX() - Nodo.D/2, nodo.getY() - Nodo.D/2, Nodo.D, Nodo.D).contains(e.getPoint())){
                    if (p1 == null)
                        ...
                        String nombre = JOptionPane.showInputDialog("Arista");
    ...
    public void mousePressed(MouseEvent e) {
       int iN = 0; 
        for (Nodo nodo : vectorNodos) {
            if(new Rectangle(nodo.getX() - Nodo.D/2, nodo.getY() - Nodo.D/2, 
                    Nodo.D, Nodo.D).contains(e.getPoint())){
                nodoMover = nodo; 
                iNodo = iN;
                break;
            }
    ...
    public void mouseDragged(MouseEvent e) {
        if (nodoMover != null){
            ...
            int iE = 0; 
            for (Enlace enlace :  vectorEnlaces) {
                if (new Rectangle(enlace.getX1()-Nodo.D/2, enlace.getY1()-Nodo.D/2,
                        Nodo.D, Nodo.D).contains(e.getPoint())){
                    this.vectorEnlaces.set(iE, new Enlace(e.getX(), e.getY(), 
                            enlace.getX2(), enlace.getY2(), enlace.getNombre()));
                }
                else if (new Rectangle(enlace.getX2()-Nodo.D/2, enlace.getY2()-Nodo.D/2,
                        Nodo.D, Nodo.D).contains(e.getPoint())){
                    this.vectorEnlaces.set(iE, new Enlace(enlace.getX1(), enlace.getY1(),
                            e.getX(), e.getY(), enlace.getNombre()));
                }
                iE++;
            }
        }
